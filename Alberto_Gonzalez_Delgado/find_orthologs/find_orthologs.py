...
# Adapted from: https://github.com/ahishsujay/Find_Orthologs
...
def get_reciprocal_hits(file_one,file_two, output_file,input_sequence_type,target_seqs,max_hsps):
...
#max target and max_hsps 1
        ...
        result_1 = "blastp -db tmp/db1 -query " + str(file_two) + " -max_target_seqs " +str(target_seqs) +" -max_hsps " +str(max_hsps) +" -outfmt 6 -out "+str(output_file) #Output for query = input2 and db = input1
        result_1_subprocess = subprocess.check_output(result_1.split())

        file_2_db = "makeblastdb -in " + str(file_two) + " -dbtype prot -out tmp/db2"   #Creating db for input2
        file_2_db_subprocess = subprocess.check_output(file_2_db.split())

        result_2 = "blastp -db tmp/db2 -query " + str(file_one) + " -max_target_seqs " +str(target_seqs) +" -max_hsps " +str(max_hsps) +" -outfmt 6 -out "+str(output_file) #Output for query = input1 and db = input2
        result_2_subprocess = subprocess.check_output(result_2.split())
...
def main():
    ...
    output_list = get_reciprocal_hits(file_one, file_two, output_file, input_sequence_type, target_seqs, max_hsps)
